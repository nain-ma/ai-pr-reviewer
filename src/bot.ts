...
`
      ...
      const err =
        "Unable to initialize the OpenAI API, both 'DIFY_API_KEY' environment variable are not available"
      throw new Error(err)
    }
  }
  ...
  chat = async (message: string, ids: Ids): Promise<[string, Ids]> => {
    ...
    try {
      ...
      res = [await this.chat_dify(message), {}]
      return res
    } catch (e: unknown) {
...
`
      ...
      if (e instanceof Error) {
        throw new Error(`Failed to chat: ${e.message}, backtrace: ${e.stack}`)
      }
      throw e
    }
  }
